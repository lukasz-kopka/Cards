...
public sealed class CardService : BaseSerivce, ICardService
{
    private readonly Dictionary<string, Dictionary<string, CardDetails>> _userCards;

    public CardService(ILogger logger) : base(logger)
    {
    ...
    public async Task<CardDetails?> GetCardDetails(string userId, string cardNumber, CancellationToken cancellationToken)
    {
        ...
        {
            Logger.Information(LoggerTemplates.CardDetailNotFound, userId, cards);
            return null;
        }
        return cardDetails;
    }
